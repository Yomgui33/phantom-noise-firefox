// ============================================================================
// Phantom Noise — Popup UI Controller (Firefox)
// ============================================================================
...
(() => {
  ...
  function setupSettings() {
    ...
    $('reset-settings').addEventListener('click', async () => {
      if (confirm('Reset all settings to defaults?')) {
        await browser.storage.local.remove('phantomState');
        await fetchState();
        syncSettings();
      }
    });
  ...
  function syncSettings() {
    ...
    // Update private browsing status indicator
    ...
    if (statusEl) {
      if (!s.useIncognito && currentState.hiddenWindowId) {
        ...
        statusEl.textContent = 'Private browsing unavailable — using hidden window with cleanup instead';
        if (helpEl) helpEl.style.display = 'block';
      } else if (!s.useIncognito) {
        statusEl.className = 'incognito-status off';
        statusEl.textContent = 'Private browsing disabled — using hidden window with cleanup';
        if (helpEl) helpEl.style.display = 'none';
      } else if (currentState.privateWindowId) {
        statusEl.className = 'incognito-status ok';
        statusEl.textContent = 'Private window active — decoy tabs are fully private';
        if (helpEl) helpEl.style.display = 'none';
      } else if (currentState.running) {
        statusEl.className = 'incognito-status warn';
        statusEl.textContent = 'Private browsing not available — check the steps below to enable it';
        if (helpEl) helpEl.style.display = 'block';
      } else {
        statusEl.className = 'incognito-status off';
        statusEl.textContent = 'Private browsing will be tested when the engine starts';
        if (helpEl) helpEl.style.display = 'block';
      }
    }
