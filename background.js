// ============================================================================
// Phantom Noise — Background Script (Firefox / Manifest V2)
...
function addLog(type, data) {
  ...
  browser.runtime.sendMessage({ action: 'log', entry }).catch(() => {});
}
...
async function loadState() {
  const data = await browser.storage.local.get('phantomState');
...
async function getOrCreateHiddenWindow() {
  // Try private window first if enabled
  if (state.settings.useIncognito) {
    if (state.privateWindowId) {
      try {
        const win = await browser.windows.get(state.privateWindowId);
        if (win) {
          await buryWindow(state.privateWindowId);
          return state.privateWindowId;
        }
      } catch {
        state.privateWindowId = null;
      }
    }
    ...
    try {
      ...
      state.privateWindowId = win.id;
      await buryWindow(win.id);
      addLog('system', { message: 'Opened hidden private window', windowId: win.id });
      return win.id;
    } catch (e) {
      addLog('system', {
        message: 'Private browsing unavailable — falling back to hidden window',
        detail: e.message
      });
  ...
  if (state.hiddenWindowId) {
    try {
      const win = await browser.windows.get(state.hiddenWindowId);
      if (win) {
...
async function triggerBehavior(tabId) {
  ...
  try {
    await browser.tabs.sendMessage(tabId, { action: 'simulate', intensity: state.intensity });
  } catch {
    setTimeout(async () => {
      try {
        await browser.tabs.sendMessage(tabId, { action: 'simulate', intensity: state.intensity });
...
function broadcastState() {
  browser.runtime.sendMessage({
    ...
    state: {
      ...
      privateWindowId: state.privateWindowId,
...
browser.windows.onRemoved.addListener((windowId) => {
  if (windowId === state.privateWindowId) {
    state.privateWindowId = null;
...
// If the hidden window ever gets focused, immediately re-minimize it
browser.windows.onFocusChanged.addListener(async (windowId) => {
  if (windowId === browser.windows.WINDOW_ID_NONE) return;
  if (windowId === state.privateWindowId || windowId === state.hiddenWindowId) {
